...
|---|---|
| 通用 | 获取当前日期时间 |
| 学生 | 搜索课程（关键词/学分/类型/教师）、推荐选修课、我的成绩（含加权平均/最低分）、我的课表、课程评价汇总 |
| 教师 | 所授课程、班级成绩分布（优/良/中/及格/不及格+均分）、学生全科成绩（按学号/姓名）、课程学生名单、授课范围内搜学生 |
| 管理员 | 系统运营统计/课程热度 TOP5、异常检测（空闲课程/无成绩记录）、最近操作日志、当前学期 |

以上全部在 `AssistantDataTools` 中实现（每条 `@Tool` 一个方法，JdbcTemplate 直查业务库，失败均返回可读提示不中断对话）。

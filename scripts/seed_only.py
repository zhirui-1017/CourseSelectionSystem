# -*- coding: utf-8 -*-
"""Generate realistic INSERT SQL and execute via MySQL"""
...
for t in ['course_evaluation','course_selection','announcement','course','class_info','major','department','college','teacher','student','admin','sys_role_permission','sys_user_role','sys_permission','sys_role','sys_user','system_config','system_log','semester']:
    L.append(f'TRUNCATE TABLE {t};')

# 2. Truncate & re-insert static data
L.append("INSERT INTO college VALUES(1,'计算机与信息技术学院','CS','',1,NOW(),NOW()),(2,'数学与统计学院','MATH','',1,NOW(),NOW()),(3,'外国语学院','FL','',1,NOW(),NOW()),(4,'经济管理学院','EM','',1,NOW(),NOW());")
L.append("INSERT INTO department VALUES(1,'CS01','计算机科学系',1,'',1,NOW(),NOW()),(2,'CS02','软件工程系',1,'',1,NOW(),NOW()),(3,'CS03','网络工程系',1,'',1,NOW(),NOW()),(4,'MATH01','数学系',2,'',1,NOW(),NOW()),(5,'MATH02','统计系',2,'',1,NOW(),NOW()),(6,'FL01','英语系',3,'',1,NOW(),NOW()),(7,'EM01','管理系',4,'',1,NOW(),NOW());")
L.append("INSERT INTO major VALUES(1,'CS0101','计算机科学与技术',1,'',1,NOW(),NOW()),(2,'CS0201','软件工程',2,'',1,NOW(),NOW()),(3,'CS0301','网络工程',3,'',1,NOW(),NOW()),(4,'MATH0101','数学与应用数学',4,'',1,NOW(),NOW()),(5,'MATH0201','统计学',5,'',1,NOW(),NOW()),(6,'FL0101','英语',6,'',1,NOW(),NOW()),(7,'EM0101','工商管理',7,'',1,NOW(),NOW());")
L.append("INSERT INTO sys_role VALUES(1,'管理员','ROLE_ADMIN','系统管理员',1,NOW(),NOW()),(2,'教师','ROLE_TEACHER','教师用户',1,NOW(),NOW()),(3,'学生','ROLE_STUDENT','学生用户',1,NOW(),NOW());")
L.append("INSERT INTO sys_permission VALUES(1,'用户管理','user:manage','/api/v1/users/**',NULL,3,NULL,NULL,0,1,NOW(),NOW()),(2,'课程管理','course:manage','/api/v1/courses/**',NULL,3,NULL,NULL,0,1,NOW(),NOW()),(3,'选课管理','selection:manage','/api/v1/selections/**',NULL,3,NULL,NULL,0,1,NOW(),NOW()),(4,'成绩管理','grade:manage','/api/v1/grades/**',NULL,3,NULL,NULL,0,1,NOW(),NOW()),(5,'系统管理','system:manage','/api/v1/system/**',NULL,3,NULL,NULL,0,1,NOW(),NOW());")
L.append("INSERT INTO sys_role_permission VALUES(1,1),(1,2),(1,3),(1,4),(1,5),(2,2),(2,4),(3,3);")
L.append("INSERT INTO semester VALUES(1,'2024-2025-1','2024-2025学年第一学期','2024-09-01','2025-01-15',0,1,NOW(),NOW()),(2,'2024-2025-2','2024-2025学年第二学期','2025-02-17','2025-07-05',0,1,NOW(),NOW()),(3,'2025-2026-1','2025-2026学年第一学期','2025-09-01','2026-01-15',1,1,NOW(),NOW());")
L.append("INSERT INTO system_config VALUES(1,'system.name','网上选课系统','系统名称',NOW(),NOW()),(2,'system.version','2.0.0','系统版本',NOW(),NOW()),(3,'contact.email','admin@example.com','联系邮箱',NOW(),NOW()),(4,'login.timeout','60','登录超时(分钟)',NOW(),NOW()),(5,'max.upload.size','50','最大上传大小(MB)',NOW(),NOW()),(6,'system.status','open','选课状态',NOW(),NOW()),(7,'grade.entry.status','open','成绩录入状态',NOW(),NOW()),(8,'max.course.selection','10','最大选课数量',NOW(),NOW()),(9,'system.notice','欢迎使用网上选课系统！','系统通知',NOW(),NOW());")
L.append(f"INSERT INTO admin VALUES(1,'admin','{BCRYPT}','系统管理员','','admin',1,NOW(),NOW());")

# 3. Classes
used = set()
cl_rows = []
for i,(code,nm,co,ma) in enumerate(CL):
    ht = gn('M')
    while ht in used: ht = gn('M')
    used.add(ht)
    cl_rows.append(f"({i+1},'{code}','{nm}',{co},{ma},'2021级','{ht}',18,'normal',NOW(),NOW())")
L.append('INSERT INTO class_info VALUES\n' + ',\n'.join(cl_rows) + ';')
...
L.append('INSERT INTO teacher VALUES\n' + ',\n'.join(te_rows) + ';')
...
L.append('INSERT INTO student VALUES\n' + ',\n'.join(st_rows) + ';')

# 6. Courses
used_slots = set()
co_rows = []
for i,(code,nm,ct,cr,hr) in enumerate(CO):
    cid = i+1
    tid = random.randint(1,12) if code[:2] in ('CS','MA','PH') else (random.randint(13,14) if code[:2]=='EN' else random.randint(15,18))
    slot = random.choice(TIMES)
    while slot in used_slots: slot = random.choice(TIMES)
    used_slots.add(slot)
    st = 1 if random.random()<0.85 else 0
    co_rows.append(f"({cid},'{code}','{nm}','{ct}',{cr},{hr},{tid},'{SEM}','{slot}','{random.choice(ROOMS)}',{random.choice([30,35,40,45,50,60,80,100])},0,{st},'本课程介绍{nm}的基本原理和方法。',NOW(),NOW())")
L.append('INSERT INTO course VALUES\n' + ',\n'.join(co_rows) + ';')
...
L.append('INSERT INTO course_selection VALUES\n' + ',\n'.join(sel_rows) + ';')
...
L.append('INSERT INTO course_evaluation VALUES\n' + ',\n'.join(ev_rows) + ';')

# 9. Update counts
for cid, cnt in sorted(Counter(int(s.split(',')[2]) for s in sel_rows).items()):
    L.append(f'UPDATE course SET selected_count = {cnt} WHERE id = {cid};')

# 10. sys_user
u_rows = [f"(1,'admin','{BCRYPT}','系统管理员',3,'ADMIN001',NULL,1,NOW(),NOW())"]
for i in range(18):
    nm = te_rows[i].split("','")[2]
    gc = '1' if te_rows[i].split("','")[3]=='男' else '2'
    u_rows.append(f"({i+2},'T{i+1:03d}','{BCRYPT}','{nm}',2,'T{i+1:03d}',{gc},1,NOW(),NOW())")
for i in range(240):
    sno = st_rows[i].split("','")[1]
    nm = st_rows[i].split("','")[2]
    gc = '1' if st_rows[i].split("','")[3]=='男' else '2'
    u_rows.append(f"({i+22},'{sno}','{BCRYPT}','{nm}',1,'{sno}',{gc},1,NOW(),NOW())")
L.append('INSERT INTO sys_user VALUES\n' + ',\n'.join(u_rows) + ';')
ur_rows = ['(1,1)'] + [f'({i},2)' for i in range(2,22)] + [f'({i},3)' for i in range(22,262)]
L.append('INSERT INTO sys_user_role VALUES\n' + ',\n'.join(ur_rows) + ';')
L.append('SET FOREIGN_KEY_CHECKS = 1;')

# Write to file
sql = '\n'.join(L)
tmp = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'db', '_seed_data.sql')
...
env = os.environ.copy()
env['MYSQL_PWD'] = 'root'
r = subprocess.run(['cmd', '/c', f'mysql -u root --ssl-mode=DISABLED --default-character-set=utf8mb4 course_selection_system_cloud < {tmp}'], capture_output=True, env=env)

if r.returncode == 0:
    os.remove(tmp)
    print(f'✅ 成功写入 course_selection_system_cloud')
    print(f'📊 班级:{len(cl_rows)} 教师:{len(te_rows)} 学生:{len(st_rows)} 课程:{len(co_rows)} 选课:{len(sel_rows)} 评价:{len(ev_rows)} 用户:261')
else:
